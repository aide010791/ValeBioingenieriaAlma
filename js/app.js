...
// Funcion para obtener el contador actual de localStorage o inicializarlo si no existe
function obtenerContadorSecuencia() {
    // Intenta obtener el contador de localStorage
    const contadorGuardado = localStorage.getItem('contadorSecuencia');
    // Si existe, conviÃ©rtelo a nÃºmero y devuÃ©lvelo; si no, inicialÃ­zalo a 1
    return contadorGuardado ? parseInt(contadorGuardado, 10) : 1;
}

// Funcion para incrementar el contador y guardarlo en localStorage
function incrementarContadorSecuencia() {
    let contadorSecuencia = obtenerContadorSecuencia();
    localStorage.setItem('contadorSecuencia', ++contadorSecuencia);
    return contadorSecuencia;
}

// Funcion para generar un ID con el valor de Boleta y un contador secuencial almacenado en localStorage
function generarId() {
    const boleta = document.getElementById('boleta').value;
    const secuencia = String(incrementarContadorSecuencia()).padStart(3, '0');
    return `${boleta}${secuencia}`;
}
